namespace NetDiretoAoPonto.TrabalhandoComDateTime
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region DateTime Formatação de datas.
            Console.WriteLine("Formatos: ");

            var formats = new string[] { "d", "D", "f", "F", "g", "G", "m", "o", "r", "s",
                                         "t", "T", "u", "U", "Y"};

            foreach (var format in formats)
            {
                Console.WriteLine("Date no formato {0}: {1}", format, now.ToString(format));
            ...
            var datFormat = now.ToString("MM-dd-yyyy");
